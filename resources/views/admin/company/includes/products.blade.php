...
<ul class="grid w-full gap-6 md:grid-cols-3">
    ...
    @foreach($products as $product)
        ...
        <li>
            <input type="checkbox" id="id-{{$product->slug}}" value="" data-client_id="{{$model->id}}"

                   {{($product->clients->first()
                    && $product->clients->first()->pivot->client_id == $model->id)
                        ?'checked':''}}

                   data-product_id="{{$product->id}}" data-url="{{route('switch.status')}}"
