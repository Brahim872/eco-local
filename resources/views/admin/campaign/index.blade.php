<x-admin.wrapper>
    ...
    <x-slot name="title">
        <div class="flex justify-between items-center">
            ...
            @can('campaign create')
                <x-admin.add-link href="{{ route('campaign.create') }}">
                    {{ __('Add campaign') }}
                </x-admin.add-link>
            @endcan
        </div>
    ...
    <div class="py-2">
        <div class="min-w-full border-b border-gray-200  overflow-x-auto">
            <div class="flex justify-between  items-center mb-5">
                <x-admin.grid.search action="{{ route('campaign.index') }}"/>
                    ...
                    <x-admin.grid.table>
                        <x-slot name="head">
                            <tr class="text-md font-semibold tracking-wide text-left text-gray-900 bg-gray-100 uppercase border-b border-gray-600">
                                ...
                                <x-admin.grid.th>
                                    @include('admin.includes.sort-link', ['label' => 'subject', 'attribute' => 'subject'])
                                </x-admin.grid.th>
                                ...
                                @canany(['campaign edit', 'campaign delete'])
                                    <x-admin.grid.th>
                                        {{ __('Actions') }}
                                    </x-admin.grid.th>
                                @endcanany
                                    ...
                                    <x-admin.grid.td>
                                        <div class="text-sm text-gray-900">
                                            <a href="{{route('campaign.show', $item->slug)}}"
                                               class="no-underline hover:underline text-cyan-600">{{ $item->title }}</a>
                                        </div>
                                    </x-admin.grid.td>
                                    <x-admin.grid.td>
                                        <div class="text-sm text-gray-900">
                                    ...
                                    @canany(['campaign edit', 'campaign delete'])
                                        <x-admin.grid.td style="width: 150px">
                                            <form action="{{ route('campaign.destroy', $item->id) }}" method="POST">
                                                <div class="flex">
                                                    @can('campaign edit')
                                                        <a href="{{route('campaign.edit', $item->slug)}}">
                                                            <x-icons.edit/>
                                                        </a>
                                                    @endcan
